...
namespace DIALOGUE.LogicalLines
{
    ...
    public static class LogicalLineUtils
    {
        public static class Encapsulation
        {
            ...
            private const char ENCAPULATION_START = '{';
            private const char ENCAPULATION_END = '}';
            ...
            public static bool IsEncapsulationStart(string line) => line.Trim().StartsWith(ENCAPULATION_START);
            public static bool IsEncapsulationEnd(string line) => line.Trim().StartsWith(ENCAPULATION_END);
        ...
        public static class Expressions
        {
            ...
            public static object CalculateValue(string[] expressionParts)
            {
                ...
                CalculateValue_RunDivisionAndMultiplication(operatorStrings, operands);

                CalculateValue_AdditionAndSubstraction(operatorStrings, operands);
                return operands[0];
            }

            private static void CalculateValue_RunDivisionAndMultiplication(List<string> operatorStrings, List<object> operands)
            {
                for(int i=0; i<operatorStrings.Count; i++)
                {
                    string operandString = operatorStrings[i];
                    if(operandString=="*" || operandString == "/")
                    {
                        ...
                        else
                        {
                            if(rightOperand ==0)
                            {
                                if(rightOperand == 0)
                                {
                                    Debug.LogError("Cannot devide by 0");
                                    return;
                                }
                                operands[i] = leftOperand / rightOperand;
                            }

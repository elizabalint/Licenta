...
namespace COMMANDS
{
    public class CMD_DatabaseExtension_General : CMD_DatabaseExtension
    {
        ...
        private static void LoadNewDialogueFile(string[] data)
        {
            ...
            if (file==null)
            {
                Debug.Log($"File '{filePath}' could not be loaded from dialogue files. Please ensure it exista within the '{FilePaths.resources_dialogueFiles}' resources folder");
                return;
            }
        ...
        private static IEnumerator ShowDialogueBox(string[] data)
        {
            ...
            param.TryGetValue(PARAM_SPEED, out speed, defaultValue: 1f);
            param.TryGetValue(PARAM_IMMEDIATE, out immediate, defaultValue: false);

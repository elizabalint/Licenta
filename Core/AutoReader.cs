...
namespace DIALOGUE
{
    public class AutoReader : MonoBehaviour
    {
        ...
        private const int DEFAUKT_CHARACTERS_READ_PER_SECOND = 18;
        ...
        private IEnumerator AutoRead()
        {
            ...
            while(conversationManager.isRunning)
            {
                if(!skip)
                {
                    ...
                    while (architect.isBuilding || conversationManager.isWaitingOnAutoTimer)
                        yield return null;
                    float timeToRead = Mathf.Clamp((float)architect.tmpro.textInfo.characterCount/DEFAUKT_CHARACTERS_READ_PER_SECOND, MIN_READ_TIME, MAX_READ_TIME);
